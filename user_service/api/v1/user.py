...
from user_service.common import Pagination, custom_exceptions
...
async def create_user(
    ...
    db: DBSessionDep,
    current_user: models.UserModel = Depends(get_current_active_superuser)
):
...
async def update_user(
    ...
    if current_user.id != user_id and not current_user.is_superuser:
        raise custom_exceptions.UnauthorizedException("You are not allowed to update this user", {"user_id": str(user_id)})
    return await user_service.update_user(db, user_id, user_in)


@router.delete("/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_user(user_id: UUID, db: DBSessionDep, current_user: models.UserModel = Depends(get_current_active_superuser)):
